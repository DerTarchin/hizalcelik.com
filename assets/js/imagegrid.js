	...
	App.ImageGrid = function(images, container, rowTargetHeight, borderOffset) {
		...
		this.fitImagesInRow = function(images) {
			while(this.getCumulativeWidth(images) > this.maxWidth) {
				for(var i = 0; i < images.length; i++) {
					images[i] = this.makeSmaller(images[i],.5);
				}
			};
		...
		this.renderGrid = function(rows) {
			...
			for(var i = 0; i < rows.length; i++) {
				...
				for(var n = 0; n < rows[i].length; n++) {
					var image = rows[i][n];
					var percwidth = (Math.ceil(image.width)*100)/this.maxWidth;
					output += '<img class="b-lazy" data-src="' + image.image + '" src="/assets/media/black.jpg" style="width:' + Math.ceil(image.width) + 'px; height:' + image.height + 'px;" />';
					// output += '<img class="b-lazy" data-src="' + image.image + '" src="/assets/media/black.jpg" style="width:' + Math.ceil(image.width) + 'px;" />';
					// output += '<img class="b-lazy" data-src="' + image.image + '" src="/assets/media/black.jpg" style="width:' + percwidth + '%" />';
				}
				output += '</div>';
			}
			return output;
